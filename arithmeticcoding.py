...
class ArithmeticCoderBase:
	...
	def update(self, cumulative_freqs, symbol):
		# State check
		low = self.low
		high = self.high
		range = high - low + 1
		...
		total = cumulative_freqs.item(-1)
		symlow = cumulative_freqs.item(symbol)
		symhigh = cumulative_freqs.item(symbol+1)
...
class ArithmeticEncoder(ArithmeticCoderBase):
	...
	def write(self, cumulative_freqs, symbol):
		self.update(cumulative_freqs, symbol)
...
class ArithmeticDecoder(ArithmeticCoderBase):
	...
	def read(self, cumulative_freqs, alphabet_size):
		...
		while end - start > 1:
			middle = (start + end) >> 1
			if cumulative_freqs[middle] > value:
				end = middle
			else:
				start = middle
